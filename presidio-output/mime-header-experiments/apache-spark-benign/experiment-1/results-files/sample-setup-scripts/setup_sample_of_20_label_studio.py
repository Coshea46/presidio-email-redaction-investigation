...
def filter_json_by_ids(txt_filepath, input_json_filepath, output_json_filepath):
    # 1. Read and clean the target IDs from the text file
    # We use a set for faster O(1) lookups
    with open(txt_filepath, 'r', encoding='utf-8') as f:
        # .strip() removes standard spaces, newlines, and non-breaking spaces
    ...
    for entry in original_data:
        # Safely extract the email_id from the nested dictionary structure
        email_id = entry.get("data", {}).get("email_id")
...
if __name__ == "__main__":
    ...
    filter_json_by_ids(TXT_FILE, INPUT_JSON, OUTPUT_JSON)